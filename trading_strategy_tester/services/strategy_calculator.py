...
class StrategyCalculator:
    ...
    def __init__(self, parameters: StrategyParameters) -> None:
        ...
        self.amount_in_shares: Decimal = Decimal("0")
        self.overall_result: Decimal = Decimal("0")
        self.buy_count: int = 0
        self.sell_count = 0
        self.comiss_sum = Decimal("0")
        self.tax_sum = Decimal("0")
        self.total_tax = Decimal("0")
        self.data_list: list[TradingResult] = []
        self.years_list: list[int] = []
    ...
    def round_money(cls, value: Decimal) -> Decimal:
        ...
        return value.quantize(cls.MONEY_PRECISION, rounding=cls.ROUNDING_METHOD)

    def _process_buy(self, min_price: Decimal) -> None:
        """
        ...
        if self.cache >= buy_price and buy_price >= min_price:
            ...
            if self.cache < count * buy_price + comiss_tmp:
                while count > 0 and self.cache < count * buy_price + comiss_tmp:
                    count -= 1
                comiss_tmp = count * buy_price * commission_rate
    ...
    def _process_sell(self, max_price: Decimal) -> Decimal:
        ...
        if max_price >= sell_price and self.share_count > 0:
            ...
            tax_tmp = self.round_money(price_differ * self.share_count * tax_rate)
    ...
    def _process_year_end_tax(self, current_date: datetime) -> None:
        """
        ...
        if year not in self.years_list and month == 12 and 20 <= day <= 31:
            self.years_list.append(year)
            self.cache -= self.tax_sum
            self.tax_sum = Decimal("0")

    def _create_trading_result(self, row: StockCandle, tax_tmp: Decimal) -> TradingResult:
        """
        Создание объекта TradingResult для текущего дня.
        """

        if not isinstance(row.close, Decimal):
            raise ValueError("row.close должен быть экземпляром класса Decimal")

        closing_price = row.close

        if not isinstance(row.begin, str):
            raise ValueError("Дата должна быть в строковом формате")
        ...
        if not isinstance(row.high, Decimal):
            raise ValueError("row.high должен быть экземпляром класса Decimal")

        if not isinstance(row.low, Decimal):
            raise ValueError("row.low должен быть экземпляром класса Decimal")
        ...
        return TradingResult(date_str=date_str, share_count=self.share_count, **rounded_values)

    def calculates_data(self, data: list[StockCandle]) -> tuple[list[TradingResult], list[int]]:
        """
        ...
        if len(data) == 0:  # Можно просто  if data: но лучше явно так описать
            logger.error("Входные данные равны None.")
            return [], []

        for row in data:
            current_date = datetime.strptime(str(row.begin).split()[0], "%Y-%m-%d")
            tax_tmp = Decimal("0")
            ...
            can_buy = self.cache >= self.parameters.buy_price and self.parameters.buy_price >= row.low
            can_sell = self.parameters.sell_price <= row.high and self.share_count > 0
            ...
            if can_buy:
                ...
                if self.parameters.sell_price <= row.high and self.share_count > 0:
